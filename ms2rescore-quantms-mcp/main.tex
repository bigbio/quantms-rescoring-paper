...
\subsection{Rescoring Features and Postprocessing}
 The quantms-rescoring Python package integrates MS2Rescore and calculates a wide range of features based on the DeepLC-predicted and observed retention times, and the MS2PIP-predicted and observed MS2 peak intensities. Signal-to-noise ratio (SNR) features are also calculated for specific cases in the quantms-rescoring Python package. More importantly, to improve compatibility and performance, we have implemented new strategies for model selection, feature export and even model training in the quantms-rescoring package. For RT-related features, retraining DeepLC model is allowed and selecting optimal model with lower MAE in calibration PSMs. The calibration PSMs are selected from each runs in a certain ratio. For MS2 intensity-related features, we implements overfitting experiment in calibration PSMs that are top-scoring 60\% non-decoy PSMs. quantms-rescoring will find the best MS²PIP model for a batch of PSMs by comparing the correlation of the PSMs with the different models. More importantly, quantms-rescoring checks if the MS²PIP results meet the specified correlation threshold and score criteria. It first filters out decoy PSMs, sorts the results based on the PSM score, and selects a calibration set. The method then verifies if at least 80\% of the calibration set has a correlation above the given threshold. we just don't do ms2pip unless forced it if all models picked is bad. It helps the user to avoid providing wrong models and also filters pre-trained models that do not fit the data. In addition, the user can also specify a list to export the specified feature scores. For these steps, each Runs is fully parallelized job, which dramatically reduces the runtime on large-scale data.
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|c|}
		...
		\hline
		PDC000127 & Comet\&MSGF+\&MS2Rescore & 11h28m & 30G & 7 \\
		PDC000127 & Comet\&MSGF+ & 6h28m & 8G & 4 \\
		PXD019643 & Comet\&MSGF+\&MS2Rescore & 17h07m & 20G & 8 \\
